...
type Token struct {
	...
	RoutingMode        string           `json:"routing_mode" gorm:"type:varchar(16);not null;default:'single';index"`
	RoutingRevision    int64            `json:"routing_revision" gorm:"type:bigint;not null;default:0"`
	RouteSteps         []TokenRouteStep `json:"route_steps,omitempty" gorm:"-"`
...
func (token *Token) Update() (err error) {
	...
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		...
		"planned_subscription_effective", "virtual_membership_id", "virtual_membership_mode").Updates(token).Error
	return err
}
...
func DeleteTokenById(id int, userId int) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("token_id = ? AND user_id = ?", id, userId).Delete(&TokenRouteStep{}).Error; err != nil {
			return err
		}
...
func BatchDeleteTokens(ids []int, userId int) (int, error) {
	...
	if err := tx.Where("user_id = ? AND token_id IN (?)", userId, ids).Delete(&TokenRouteStep{}).Error; err != nil {
		tx.Rollback()
		return 0, err
	}
