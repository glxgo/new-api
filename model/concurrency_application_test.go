...
func TestConcurrencyApplicationLifecycle(t *testing.T) {
	...
	application, err := CreateConcurrencyApplication(user.Id, 400, "团队多人同时使用，需要提升并发容量", "contact@example.com")
	require.NoError(t, err)
	require.Equal(t, 200, application.CurrentLimit)
	...
	_, err = CreateConcurrencyApplication(user.Id, 500, "第二个同时提交的申请应被拒绝", "contact@example.com")
	require.Error(t, err)

	reviewed, err := ReviewConcurrencyApplication(application.Id, 99, true, 300, "approved")
	require.NoError(t, err)
	require.Equal(t, ConcurrencyApplicationApproved, reviewed.Status)
	var refreshed User
	require.NoError(t, db.First(&refreshed, user.Id).Error)
	require.Equal(t, 300, refreshed.ConcurrencyLimit)
