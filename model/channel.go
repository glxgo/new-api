...
type Channel struct {
	...
	// Unbounded string maps to LONGTEXT on MySQL and TEXT on PostgreSQL/SQLite.
	// The list has no business limit on group count; defaults are applied in BeforeCreate.
	...
	// CostRatioPPM is the channel-level accounting cost multiplier in fixed point.
	// 1_000_000 means 1.0x. nil means "not configured"; zero is an explicit zero-cost channel.
	CostRatioPPM       *int64 `json:"cost_ratio_ppm" gorm:"column:cost_ratio_ppm;default:null"`
...
func GetAllChannelsWithoutKey() ([]*Channel, error) {
	var channels []*Channel
	err := DB.Omit("key").Order(clause.OrderByColumn{
		Column: clause.Column{Name: "priority"},
		Desc:   true,
	}).Find(&channels).Error
	return channels, err
...
// ApplyChannelSearchFilter keeps legacy name/id/key/base URL and model matching
// shared by list/count queries. It does not select, order or materialize rows.
func ApplyChannelSearchFilter(query *gorm.DB, keyword, modelKeyword string) *gorm.DB {
	return query.Where(
		"(id = ? OR name LIKE ? OR "+commonKeyCol+" = ? OR base_url LIKE ?) AND models LIKE ?",
		common.String2Int(keyword), "%"+keyword+"%", keyword, "%"+keyword+"%", "%"+modelKeyword+"%",
	)
}
...
func SearchChannels(keyword string, group string, model string, idSort bool, sortOptions ...ChannelSortOptions) ([]*Channel, error) {
	var channels []*Channel
	order := resolveChannelSortOptions(idSort, sortOptions)
	baseQuery := ApplyChannelGroupFilter(ApplyChannelSearchFilter(DB.Model(&Channel{}).Omit("key"), keyword, model), group)
...
func EditChannelByTag(tag string, newTag *string, modelMapping *string, models *string, group *string, priority *int64, weight *uint, costRatioPPM *int64, paramOverride *string, headerOverride *string) error {
	updateData := Channel{}
	...
	if costRatioPPM != nil {
		updateData.CostRatioPPM = costRatioPPM
	}
