...
export const useLogsData = () => {
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      // The request format conversion is safe for the log owner. Channel IDs,
      // retry chains, affinity keys and billing-source diagnostics below stay
      // administrator-only.
      if (logs[i].type !== 6 && logs[i].type !== 1) {
        expandDataLocal.push({
          key: t('请求转换'),
