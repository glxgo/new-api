...
func TestApplyPrioritySurcharge(t *testing.T) {
	...
	tests := []struct {
		...
		{"openai priority tiered expr is expression-owned", constant.ChannelTypeOpenAI, "priority", `len <= 272000 ? tier("standard", p*2.5+c*15) : tier("long_context", p*5+c*30)`, 100, 100, false},
		{"openai priority tiered expr that already handles service_tier skipped", constant.ChannelTypeOpenAI, "priority", `tier("base", p*5+c*25)|||when(param("service_tier")=="priority")*2`, 100, 100, false},
