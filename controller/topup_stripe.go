...
func (*StripeAdaptor) RequestAmount(c *gin.Context, req *StripePayRequest) {
	...
	payMoney, discountOK := applyUserRechargeDiscount(c, getStripePayMoney(float64(req.Amount), group))
	if !discountOK {
		return
	}
	if payMoney <= 0.01 {
...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	payMoney := model.ApplyRechargeDiscount(getStripePayMoney(float64(req.Amount), user.Group), user.RechargeTotalCents)
	if payMoney < 0.01 {
...
//   - payMoney: discounted server quote before provider promotion codes
...
func genStripeLink(referenceId string, customerId string, email string, payMoney float64, successURL string, cancelURL string) (string, error) {
	...
	// Keep the configured product/currency while charging the server quote.
	configuredPrice, err := price.Get(setting.StripePriceId, nil)
	if err != nil {
		return "", err
	}
	if configuredPrice.Product == nil {
		return "", errors.New("Stripe price has no product")
	}
	snapshot, err := model.NewPaymentSnapshotFromMoney(payMoney, string(configuredPrice.Currency))
	if err != nil {
		return "", err
	}
		...
		LineItems: []*stripe.CheckoutSessionLineItemParams{
			{
				PriceData: &stripe.CheckoutSessionLineItemPriceDataParams{
					Currency: stripe.String(string(configuredPrice.Currency)), Product: stripe.String(configuredPrice.Product.ID), UnitAmount: stripe.Int64(snapshot.AmountMinor),
				},
...
func getStripePayMoney(amount float64, group string) float64 {
	...
	payMoney, _ := decimal.NewFromFloat(amount).Mul(decimal.NewFromFloat(setting.StripeUnitPrice)).Mul(decimal.NewFromFloat(topupGroupRatio)).Mul(decimal.NewFromFloat(discount)).Float64()
	return payMoney
}
