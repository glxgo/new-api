...
// CalcModelCostQuota 计算单次请求的平台成本(quota 单位)。
// 成本 = 平台付给上游(如 krill)的买入价, 由 ModelCost($/1M tokens) × tokens 换算。
// 关键: 成本【不乘分组倍率 groupRatio】——成本是平台固定承担的, 与用户所在分组无关
// (分组倍率只影响面向用户的售价)。
// 第二返回值 ok=false 表示该模型未配置成本, 此时利润无法精确计算, 该笔成本按 0 处理。
func CalcModelCostQuota(modelName string, promptTokens, completionTokens int) (int, bool) {
	...
	// 成本 = (InputCostPerM × prompt + OutputCostPerM × completion) / 1e6 × QuotaPerUnit
